...
$(function() {
    ...
    $('.xh-href').click(function() {
        ...
        if (xh_href != '') {
            ...
            var h = '<li xh-id="' + xh_href + '" >' + t_name + '<b class="fa fa-close">X</b></li>';
        ...
        // 关闭
        $('.fa-close').find('li').click(function(){
            $(this).remove();
        })
...
function xh_select() {
    var classname = $('.xh-selectpicker').attr('class');
    if (classname != undefined) {
        classname = classname.replace('xh-selectpicker', '');
        var title = $('.xh-selectpicker').attr('xh-title');
        var h = '<div class="' + classname + '">';
        h += '<button class="xh-btn xh-dropdown-toggle">';
        h += '<ul>';
        var bt = '<div class="xh-selext-text">';
        $('.xh-selectpicker option').each(function() {
            var t = $(this).text();
